...
public class QueueManager {
    ...
    private final Map<String, Queue> queueTasks = new HashMap<>();
    ...
    public int addToQueue(Collection<UUID> players, QueueType queueType) throws IllegalArgumentException {
        ...
        Queue queue = queueTasks.computeIfAbsent(queueType.getId(), s -> new Queue(intentManager, liveProfileManager, this, gameFinder, publishers, queueType));
        if (!queue.isRunning()) {
    ...
    public @Nullable Queue getQueue(QueueType queueType) {
        return queueTasks.get(queueType.getId());
    }
